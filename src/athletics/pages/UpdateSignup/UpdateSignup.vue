...
import { personSignupApplyFormApi, updateSignupApplyDataApi, getMemberSignupApplyInfoDataApi } from '@/athletics/api/sport';
import {
  getGroupList,
...
async function requestAll(key: string, ...args: any[]) {
  ...
  if (key == 'onLoad') {
    ...
    requestTasks.add(updateSignupApplyDataFn());
  }
  requestTasks.end();
}
...
async function updateSignupApplyDataFn() {
  const { teamSignupId, memberId } = optionInfo;
  ...
  const params: UpdateSignupApplyDataReq = {
    dataList,
    teamSignupId,
    userName,
    userPhone,
  ...
  const task = updateSignupApplyDataApi(params);
  task.then((res) => {
    customShowToast(res);
    if (res.success) {
      setTimeout(() => {
