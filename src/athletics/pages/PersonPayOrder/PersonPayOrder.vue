...
import { requestTasks, customShowToast, setStatisticList, createLowercasePropertyProxy } from '@/utils/index';
import { UpdateEventTeamInfoReq, OrderItem, OrderConfirmInfoRes } from '@/athletics/model/sport';
import { getSignupOrderConfirmInfoByIndividualApi } from '@/athletics/api/sport';
import { doPayApi } from '@/athletics/api/pay';
async function requestAll(key: string, ...args: any[]) {
  requestTasks.start();
  if (key == 'onShow') {
    requestTasks.add(getSignupOrderConfirmInfoByIndividualFn());
  } else if (key == 'submit') {
...
async function getSignupOrderConfirmInfoByIndividualFn() {
  const { individualSignupId } = optionInfo;
  if (!individualSignupId) {
    return;
  }
  const task = getSignupOrderConfirmInfoByIndividualApi(individualSignupId);
  task.then((res) => {
...
const groupDataList = reactive<StatisticItem[]>([
  ...
  { label: '报名人', value: '', key: 'signupUserName' }
]);
